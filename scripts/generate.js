...
try { fs.unlinkSync(level_path); } catch (e) { }
try { fs.mkdirSync(level_path); } catch (e) { }
...
function create_class(name, item) {
    fs.writeFileSync(path.join(level_path, `${name}.ts`), `export default class ${name}${item.generic || ""} {
...
function create_return_value(value) {
    switch (value){
...
function create_function(name, item) {
    ...
    fs.writeFileSync(path.join(level_path, `${name}.ts`), `export default function ${item.fn}${g}(${item.args}): ${item.return} {
        ${create_return_value(item.return)}
}`);
}
console.log(config)
